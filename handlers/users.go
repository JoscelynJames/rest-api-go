...
// Userhandler represents the handler for operating on the User resource
...
func (uh UserHandler) OneUser(w http.ResponseWriter, r *http.Request) {
	...
	u := models.User{}
	// fetch user
	if err := uh.session.DB("go-rest-service").C("users").FindId(oid).One(&u); err != nil {
		w.WriteHeader(404)
		return
	}
	// write content-type, status code, payload
...
func (uh UserHandler) AllCoords(w http.ResponseWriter, r *http.Request) {
	// json.NewEncoder(w).Encode(coordinates)
}

func (uh UserHandler) CreateUser(res http.ResponseWriter, req *http.Request) {
